...
def args_config():
    ...
    parser.add_argument('--net', type=str, default="INC_T")  # ["RES","INC","HRN",'C','M','T']
    parser.add_argument('--mode', type=str, default="trainTriplet")  # ["train","trainMixup","trainRemix","trainTriplet"]
...
def train_tri(train_loader, loss_func, mining_func, model, itr):
    ...
    for batch_idx, samples in tqdm(enumerate(train_loader), total=len(train_loader), desc=f'{args.net} trainTriplet_mode with warmup {args.warmup}'):
        ...
        # loss = 0.5 * loss1 + 0.5 * loss2 + loss3 + 2 * tri_loss1 + 2 * tri_loss2
...
if __name__ == '__main__':
    ...
    criterion_tri = losses.TripletMarginLoss(margin=0.2, distance=distance, reducer=reducer)
